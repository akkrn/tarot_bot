...
async def handle_generation_failure(bot, message, state, payment_id=None, user=None):
    ...
    async with async_session() as session:
        ...
        if  message.successful_payment.invoice_payload == "balance_topup":
            ...
            await message.answer(
                f"Баланс успешно пополнен на {amount_rub}₽. Текущий баланс: {user.balance_rub}₽"
            )
            await state.set_state(AskState.question)
